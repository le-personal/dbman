...
SSH.prototype.execute = function(id, command) {
	...
	self.connection.on("ready", function() {
		...
		self.connection.exec(command, function(err, stream) {
	      ...
	      app.emit("ssh:execute:end", {id: id});
	    });
...
SSH.prototype.testConnection = function(id) {
	...
	self.connection.on("ready", function() {
		self.connection.exec("uptime", function(err, stream) {
			if(err) throw err;
			stream.on("data", function(data, extended) {
				app.emit("ssh:testConnection:data", {id: id, data: data, type: extended});
			})

			stream.on('end', function() {
	      console.log('Stream :: EOF');

	      app.emit("ssh:testConnection:end", {id: id});
	    });

	    stream.on('close', function() {
	      console.log('Stream :: close');

	      app.emit("ssh:testConnection:close", {id: id});
	    });

	    stream.on('exit', function(code, signal) {
	      console.log('Stream :: exit :: code: ' + code + ', signal: ' + signal);
	      app.emit("ssh:testConnection:exit", {id: id});
	      self.connection.end();
	    });
		});
	})
